...
/** Command to verify the whole project stainless-dotty VerifiedRegexMatcher.scala ListUtils.scala VerifiedNFA.scala -Dparallel=3
  * --config-file=stainless.conf --watch
  */
...
object VerifiedNFA {
  ...
  def fromRegexToNfa[C](r: Regex[C]): NFA[C] = {
    ...
    ListUtils.lemmaForallContainsPreservedIfSameContent(
      transitionsStates(transitions),
      transitionsStates(ListUtils.removeDuplicates(transitions, transitions)),
      allStates
    )
    ...
    NFA(startState, List(finalState), errorState, ListUtils.removeDuplicates(transitions, transitions), Cons(errorState, allStates))
  ...
  def go[C](regex: Regex[C])(errorState: State): (State, List[State], List[Transition[C]], State) = {
    ...
    regex match {
      case EmptyLang() => {
        val stout = State("out")
        val ste = errorState
        val newStates = Cons(stout, Nil())
        val transitions: List[Transition[C]] = Nil[Transition[C]]()

        (ste, newStates, transitions, stout)
      }
      case EmptyExpr() => {
        // ListUtils.lemmaSubseqRefl(allStates)
        val ste = State("start")
        val stout = State("out")
        val states = Cons(stout, Cons(ste, Nil()))
        val transitions: List[Transition[C]] = Cons(EpsilonTransition(ste, stout), Nil())

        (ste, states, transitions, stout)
      }
      case ElementMatch(c) => {
        val ste = State("start")
        val stout = State("out")
        val newStates = Cons(stout, Cons(ste, Nil()))
        val newTransition = LabeledTransition(ste, c, stout)
        val newTransitions: List[Transition[C]] = Cons(newTransition, Nil())
        (ste, newStates, newTransitions, stout)
      ...
      case Union(r1, r2) => {
        ...
        val newTransitions: List[Transition[C]] =
          Cons(t1, Cons(t2, Cons(t3, Cons(t4, combineTransitions(transitions1, transitions2, "l", "r", errorState)))))
      ...
      case Concat(r1, r2) => {
        ...
        val newTransitions: List[Transition[C]] = Cons(t1, Cons(t2, Cons(t3, combineTransitions(transitions1, transitions2, "l", "r", errorState))))
      ...
      case Star(r) => {
        ...
        val stout = State("out")
        val ste = State("start")
        val newStates = Cons(ste, Cons(stout, renameStates(statesInner, "i")))
        ...
        val newTransitions: List[Transition[C]] = Cons(t1, Cons(t2, Cons(t3, Cons(t4, renameStatesInTransitions(transitionsInner, "i", errorState)))))
        (ste, newStates, newTransitions, stout)
      }
    }
  ...
  def combineTransitions[C](
      trs1: List[Transition[C]],
      trs2: List[Transition[C]],
      prefix1: String,
      prefix2: String,
      errorState: State
  ): List[Transition[C]] = {
    ...
    renameStatesInTransitions(trs1, prefix1, errorState) ++ renameStatesInTransitions(trs2, prefix2, errorState)
    // TODO proof

  } ensuring (res => ListSpecs.noDuplicate(res))

  def renameStatesInTransitions[C](trs: List[Transition[C]], prefix: String, errorState: State): List[Transition[C]] = {
    trs match {
      case Cons(hd, tl) => {
        hd match {
          case EpsilonTransition(from, to) => {
            val newFrom = if (from != errorState) renameState(from, prefix) else errorState
            val newTo = if (to != errorState) renameState(to, prefix) else errorState
            Cons(EpsilonTransition(newFrom, newTo), renameStatesInTransitions(tl, prefix, errorState))
          }
          case LabeledTransition(from, c, to) => {
            val newFrom = if (from != errorState) renameState(from, prefix) else errorState
            val newTo = if (to != errorState) renameState(to, prefix) else errorState
            Cons(LabeledTransition(newFrom, c, newTo), renameStatesInTransitions(tl, prefix, errorState))
          }
        }
      }
      case Nil() => Nil()
  ...
  def lemmaSameTransitionsContentOutOfErrorStatePreserved[C](
      transitions: List[Transition[C]],
      transitionsBis: List[Transition[C]],
      errorState: State
  ): Unit = {
  ...
  def lemmaAddNewTransitionPreservesForallStatesContainedIfAddingStates[C](
    ...
    !oldStates.contains(s1) && !oldStates.contains(s2) && transitionsStates(Cons(t, transitions)).forall(s =>
      Cons(s2, Cons(s1, oldStates)).contains(s)
    )
  })
    ...
    toExplore match {
      ...
      case Cons(hd, tl) => {
        if (seen.contains(hd)) {
          ...
          val reachableFromHd: List[State] =
            unseenReachableStatesThroughEpsilon(nfa.transitions, epsilonTransitionsFrom(hd, nfa.transitions), hd, seenArg, nfa.allStates)
        ...
        hd match {
          case EpsilonTransition(from, to) => {
            ...
            if (!seen.contains(to)) { // && !toExplore.contains(to)) {
              ...
              ListUtils.lemmaForallContainsAddingElmtInPreserves(
              ...
              if (unseenReachableStatesThroughEpsilon(allTransitions, tl, state, seen, allStates).contains(to)) {
                ListSpecs.forallContained(
                  unseenReachableStatesThroughEpsilon(allTransitions, tl, state, seen, allStates),
                  s => tl.contains(EpsilonTransition(state, s)),
                  to
                )
                check(false)
              }
